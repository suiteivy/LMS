...
-- 3.1 Subject Teachers (Many-to-Many join table)
...
-- ---------------------------------------------------------
-- PART 10: CUSTOM ROLE & PERMISSION MANAGEMENT
-- ---------------------------------------------------------
...
-- Trigger to automatically create role-specific profile entries when roles are assigned
...
-- Trigger to automatically sync the primary teacher of a subject to the subject_teachers table
CREATE OR REPLACE FUNCTION sync_subject_teacher()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.teacher_id IS NOT NULL THEN
        INSERT INTO subject_teachers (subject_id, teacher_id, institution_id)
        VALUES (NEW.id, NEW.teacher_id, NEW.institution_id)
...
DROP TRIGGER IF EXISTS tr_sync_subject_teacher ON subjects;
CREATE TRIGGER tr_sync_subject_teacher
AFTER INSERT OR UPDATE OF teacher_id ON subjects
FOR EACH ROW EXECUTE FUNCTION sync_subject_teacher();
