...
const DailyTab = ({ classId, className: cName }: { classId: string; className: string }) => {
    ...
    useEffect(() => {
        CalendarAPI.getCancelledDates()
            .then(setCancelledDates)
            .catch(() => setCancelledDates([]));
    }, []);
    ...
    const handleDownloadClassTimetablePdf = async () => {
        try {
            setDownloadingPdf(true);
            await downloadTimetablePdf({
                title: `${cName} Timetable`,
                subtitle: profile?.full_name ? `Generated by ${profile.full_name}` : 'Class Schedule',
                institutionName,
                institutionLogo,
                entries: schedule,
                cancelledDates,
                referenceDate: date,
                fileName: `${cName}-timetable-${isoDate(date)}`,
            });
            Alert.alert('PDF Ready', 'Class timetable PDF generated successfully.');
        } catch {
            Alert.alert('Export failed', 'Failed to generate class timetable PDF.');
        } finally {
    ...
    return (
        <View className="flex-1">
            ...
            <View className="flex-row justify-end mb-3">
                <TouchableOpacity
                    onPress={handleDownloadClassTimetablePdf}
                    disabled={downloadingPdf}
                    className="px-3 py-1.5 rounded-xl border border-orange-200 bg-orange-50"
                    accessibilityRole="button"
                    accessibilityLabel="Download class timetable PDF"
                >
