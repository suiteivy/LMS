            ...
            >
                ...
                <View className="p-4 md:p-8">
                    ...
                    <View className="flex-row gap-3 mb-8">
                        <View className="flex-1 flex-row items-center bg-[#F6F8FA] dark:bg-[#161B22] px-5 py-3.5 rounded-lg border border-[#D0D7DE] dark:border-[#21262D]">
                        ...
                        <TouchableOpacity className="w-14 h-14 bg-[#F6F8FA] dark:bg-[#161B22] rounded-lg items-center justify-center border border-[#D0D7DE] dark:border-[#21262D] active:bg-gray-50">
                            <Filter size={20} color="#FF6900" />
                        </TouchableOpacity>
                    </View>

                    {loading ? (
                        <ListItemSkeleton loading={loading} count={4} label="Loading library data..." />
                    ) : (
                        <ScrollView
                            className="flex-1"
                            ...
                            {viewMode === 'personal' ? (
                                <>
                                    ...
                                    {borrowingHistory.filter(b => ['borrowed', 'overdue'].includes(b.status)).length > 0 && (
                                        <>
                                            ...
                                            {borrowingHistory.filter(b => ['borrowed', 'overdue'].includes(b.status)).map((borrow) => (
                                                <View key={borrow.id} className="bg-[#F6F8FA] dark:bg-[#161B22] p-5 rounded-xl border border-[#D0D7DE] dark:border-[#21262D] mb-3 flex-row items-center">
                                                    <View className="w-12 h-12 rounded-lg bg-orange-50 dark:bg-orange-950/20 items-center justify-center mr-4">
                                    ...
                                    {filteredBooks.length === 0 ? (
                                        <View className="bg-[#F6F8FA] dark:bg-[#161B22] p-12 rounded-[40px] items-center border border-[#D0D7DE] dark:border-[#21262D] border-dashed mt-4">
                                ...
                                <>
                                    ...
                                    {studentLoans.filter(l => l.borrowerType === 'student' && l.status !== 'returned').length === 0 ? (
                                        ...
                                        studentLoans.filter(l => l.borrowerType === 'student' && l.status !== 'returned').map((loan) => (
                                            <View key={loan.id} className="bg-[#F6F8FA] dark:bg-[#161B22] p-5 rounded-xl border border-[#D0D7DE] dark:border-[#21262D] mb-3">
                    ...
                    <View className="flex-1 bg-black/60 justify-end">
                        <View className="bg-[#FFFFFF] dark:bg-[#161B22] rounded-t-[50px] p-8 pb-12 border-t border-[#D0D7DE] dark:border-[#21262D]">
                            ...
                            <TouchableOpacity
                                ...
                                className={`py-5 rounded-lg items-center shadow-lg active:bg-gray-800 ${actionLoading || (selectedBook?.available ?? 0) <= 0 ? 'bg-gray-300 shadow-none' : 'bg-gray-900'}`}
                                onPress={() => {
                            ...
                            {isIssuing && (
                                <View className="mt-8 border-t border-[#D0D7DE] dark:border-[#21262D] pt-8">
                                    ...
                                    {selectedStudent && (
                                        <View className="bg-orange-50 dark:bg-orange-950/20 p-4 rounded-lg flex-row justify-between items-center mb-4">
                                    ...
                                    <TouchableOpacity
                                        ...
                                        className={`py-5 rounded-lg items-center ${!selectedStudent ? 'bg-gray-200' : 'bg-[#FF6900]'}`}
                                    >
