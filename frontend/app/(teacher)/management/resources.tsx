...
const ResourceCard = ({ resource, onDelete }: { resource: Resource; onDelete: (id: string) => void }) => {
  ...
  return (
    <View className="bg-white dark:bg-[#161B22] p-5 rounded-xl border border-gray-100 dark:border-white/10 mb-4">
      ...
      <View className="flex-row items-center">
        ...
        <View className="flex-row gap-2">
          <TouchableOpacity className="w-10 h-10 bg-gray-50 dark:bg-white/5 rounded-xl items-center justify-center" onPress={() => Alert.alert("Download", `Opening ${resource.url}`)}>
            <Download size={18} color="#6B7280" />
          </TouchableOpacity>
          <TouchableOpacity className="w-10 h-10 bg-red-50 dark:bg-red-500/10 rounded-xl items-center justify-center" onPress={() => onDelete(resource.id)}>
...
export default function ResourcesPage() {
  ...
  const handleAddResource = async () => {
    ...
    if (isDemo) {
      ...
      newResource.Subject_title = Subjects.find(s => s.id === selectedSubjectId)?.title || "Selected Subject";
    ...
    <View className="flex-1 bg-[#F6F8FA] dark:bg-[#161B22]">
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        <View className="p-4 md:p-8">
          ...
          <View className="flex-row justify-between items-center mb-6 px-2">
            ...
            <TouchableOpacity
              className="flex-row items-center bg-gray-900 px-5 py-2.5 rounded-lg shadow-lg active:bg-gray-800"
          ...
          {/* Storage Info */}
          <View className="bg-white dark:bg-[#161B22] p-6 rounded-[40px] border border-gray-100 dark:border-white/10 mb-8">
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-white dark:bg-[#161B22] rounded-t-[40px] p-8 pb-12">
            <View className="flex-row justify-between items-center mb-8">
              <Text className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Add Resource</Text>
              <TouchableOpacity
                className="w-10 h-10 bg-gray-50 dark:bg-white/5 rounded-full items-center justify-center"
