...
export default function StudentRankingsScreen() {
  ...
  const filteredRankings = useMemo(() => {
    ...
    return data.rankings.filter((r) => r.cbc_band === selectedBand);
  ...
  const getBadgeStyle = (band: string) => {
    switch (band) {
      ...
      case "ME":
        return { bg: isDark ? "#1E3A8A" : "#EFF6FF", text: "#3B82F6", border: "#3B82F6" };
      case "AE":
        return { bg: isDark ? "#78350F" : "#FEF3C7", text: "#F59E0B", border: "#F59E0B" };
      default:
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg }}>
      ...
      >
        ...
        {/* CBC Competency Distribution Cards (EE / ME / AE / BE) */}
        <Text style={{ color: textSecondary, fontSize: 11, fontWeight: "800", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8, marginLeft: 2 }}>
          CBC Competency Distribution
        </Text>
        <View style={{ flexDirection: "row", gap: 8, marginBottom: 18 }}>
          {[
            { band: "EE", label: "Exceeding", count: data?.cbc_distribution?.EE || 0, color: "#10B981" },
            { band: "ME", label: "Meeting", count: data?.cbc_distribution?.ME || 0, color: "#3B82F6" },
            { band: "AE", label: "Approaching", count: data?.cbc_distribution?.AE || 0, color: "#F59E0B" },
            { band: "BE", label: "Below", count: data?.cbc_distribution?.BE || 0, color: "#EF4444" },
          ].map((item) => {
        ...
        {loading ? (
          ...
          <View style={{ gap: 8 }}>
            {filteredRankings.map((student, idx) => {
              const badge = getBadgeStyle(student.cbc_band);
