...
export default function TeacherNotifications() {
 ...
 {unreadCount > 0 && (
 <TouchableOpacity
 className="flex-row items-center bg-white px-5 py-2.5 rounded-lg border border-gray-100 active:bg-gray-50"
 ...
 className={`p-6 rounded-[32px] mb-4 flex-row items-start border ${item.is_read ? 'bg-white border-gray-50' : 'bg-white border-orange-100'}`}
 >
 <View className={`p-3 rounded-lg mr-4 ${item.is_read ? 'bg-gray-50' : 'bg-orange-50'}`}>
 ...
 {!item.is_read && (
 <View className="w-2 h-2 bg-[#FF6900] rounded-full mt-1.5 ml-2" />
 )}
 </View>
